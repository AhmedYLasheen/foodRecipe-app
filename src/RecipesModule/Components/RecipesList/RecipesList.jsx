...
const navigateToRecipeData =()=>{
  ...
  const handleDelete = (id) => {
    ...
    // console.log(id);
    // console.log(token);
    axios.delete(`https://upskilling-egypt.com:443/api/v1/Recipe/${id}`, { headers: { Authorization: token } })
      .then((response) => {
        getList();
        handleClose();
...
<Modal show={show} onHide={handleClose}>
        ...
        <div className='delete-button d-flex justify-content-end p-3'>
            <button onClick={() => { handleDelete(RecipesId) }} className='btn btn-danger'>Delete this item</button>
          </div>
              ...
              <tbody className="">
                  ...
                  {recipesList.map((recipe)=>
                   ...
                   <td className="">
                    ...
                    {recipe.imagePath ? <img src={`https://upskilling-egypt.com/${recipe.imagePath}`} className=" table-user-img " />:
                    <img src={noData} className=" table-user-img" />
                    }
                    </td>
                   <td>{recipe.category[0].name}</td>
                   <td>
                     <i className="fas fa-edit text-warning mx-2 " aria-hidden="true"></i>
                     <i  onClick={() => {
                          handleShow(),
                          setcategoriesId(recipe.id)
                        }} className="fas fa-trash text-danger mx-2 " aria-hidden="true"></i>
                   </td>
                 </tr>
